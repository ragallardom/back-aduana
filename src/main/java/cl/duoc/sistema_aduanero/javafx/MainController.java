...
public class MainController {
    ...
    private void handleEnviarSolicitud() {
        try {
            ...
            if (nombre.isBlank() || numero.isBlank() || tipoDocumentoEnt.isBlank()
                ...
                showAlert(Alert.AlertType.WARNING, "Todos los campos de texto son obligatorios.");
                return;
            }
            if (archivoSeleccionado == null) {
                showAlert(Alert.AlertType.WARNING, "Debes seleccionar un archivo para adjuntar.");
                return;
            }

            // 2) Construir el body multipart/form-data usando MultipartBodyPublisher
            MultipartBodyPublisher bodyPublisher = MultipartBodyPublisher.builder()
            ...
            if (response.statusCode() == 201) {
                showAlert(Alert.AlertType.INFORMATION, "Solicitud enviada correctamente.");
                limpiarFormulario();
            } else {
                showAlert(Alert.AlertType.ERROR,
                        "Error al enviar solicitud (" + response.statusCode() + "):\n" + response.body());
            }
        } catch (Exception ex) {
            ex.printStackTrace();
    ...
    private void handleVerDetalle() {
        ...
        try {
            ...
            e.printStackTrace();
            showAlert(Alert.AlertType.ERROR, "Error al convertir a JSON: " + e.getMessage());
        }
    }
    ...
    private void cambiarEstado(Long id, String nuevoEstado) {
        try {
            ...
            HttpResponse<String> response = httpClient.send(request, HttpResponse.BodyHandlers.ofString());
            if (response.statusCode() == 200) {
                showAlert(Alert.AlertType.INFORMATION, "Estado actualizado a " + nuevoEstado);
                handleRefrescar(); // Refrescar la lista tras el cambio
            } else {
                showAlert(Alert.AlertType.ERROR, "Error al cambiar estado (" + response.statusCode() + "): " + response.body());
            }
        } catch (Exception ex) {
            ex.printStackTrace();
            showAlert(Alert.AlertType.ERROR, "Exception: " + ex.getMessage());
